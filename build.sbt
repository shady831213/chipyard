...
lazy val `api-config-chipsalliance` = (project in rocketChipDir / "api-config-chipsalliance/build-rules/sbt")
  .settings(commonSettings)

lazy val hardfloat = (project in rocketChipDir / "hardfloat")
  .settings(commonSettings).dependsOn(chisel) //.dependsOn(midasTargetUtils)
...
lazy val rocketchip = freshProject("rocketchip", rocketChipDir)
  .settings(commonSettings)
  .dependsOn(`api-config-chipsalliance`, chisel, hardfloat, rocketMacros)
...
lazy val example = conditionalDependsOn(project in file("generators/example"))
  .dependsOn(sifive_blocks, sifive_cache, utilities)
  .settings(commonSettings)
